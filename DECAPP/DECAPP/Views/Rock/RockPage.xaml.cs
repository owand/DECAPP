...
namespace DECAPP.Views.Rock
{
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class RockPage : ContentPage
    {
        private RockList viewModel;
        ...
        protected override async void OnAppearing()
        {
            ...
            try
            {
                ...
                BindingContext = viewModel = viewModel ?? new RockList();
        ...
        private async Task SaveHostItem()
        {
            try
            {
                if (viewModel.GetSelectHostItem() != null)
                {
                    ...
                    viewModel.SelectHostItem.ROCKNAME = viewModel.SelectItem.ROCKNAME;
                }
                else
                {
                    // Новая основная коллекция
                    viewModel.NewHostItem = new RockModel() // Данные для новой основной коллекции
                    {
                        ROCKNAME = viewModel.SelectItem.ROCKNAME
                    };
                }
        ...
        private async Task SaveSubItem()
        {
            try
            {
                ...
                else
                {
                    ...
                    viewModel.NewSubItem = new RockSubModel()
                    {
                        ROCKTYPENAME = viewModel.SelectItem.ROCKTYPENAME,
        ...
        private async void OnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    viewModel.SelectItem = viewModel.NewJoinItem != null ? viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.NewHostItem.ROCKID) :
                                                                           viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.SelectHostItem.ROCKID);
                    MasterContent.ScrollTo(viewModel.SelectItem, ScrollToPosition.Center, true); // Прокручиваем Scroll до активной записи.
                }
