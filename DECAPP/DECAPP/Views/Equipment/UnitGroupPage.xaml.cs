...
namespace DECAPP.Views.Equipment
{
    ...
    public partial class UnitGroupPage : ContentPage
    {
        private UnitGroup viewModel;

        public UnitGroupPage()
        {
            InitializeComponent();
        ...
        protected override async void OnAppearing()
        {
            ...
            try
            {
                ...
                BindingContext = viewModel = viewModel ?? new UnitGroup();
        ...
        private void OnSelection(object sender, SelectedItemChangedEventArgs e) // Загружаем дочернюю форму и передаем ей параметр ID.
        {
            try
            {
                if (e.SelectedItem != null) // Если в Collection есть записи.
                {
                    ...
                    MasterContent.ScrollTo(viewModel.SelectHostItem, ScrollToPosition.Center, true); // Прокручиваем Scroll до активной записи.
                }
                else
                {
        ...
        private async Task SaveHostItem()
        {
            try
            {
                if (viewModel.GetSelectHostItem() != null)
                {
                    ...
                    viewModel.SelectHostItem.GROUPNAME = viewModel.SelectItem.GROUPNAME;
                }
                else
                {
                    // Новая основная коллекция
                    viewModel.NewHostItem = new UnitGroupModel()
                    {
                        GROUPNAME = viewModel.SelectItem.GROUPNAME
                    };
                }
        ...
        private async Task SaveSubItem()
        {
            try
            {
                ...
                if (viewModel.GetSelectSubItem() != null)
                {
                    ...
                    viewModel.SelectSubItem.SYMBOL = viewModel.SelectItem.SYMBOL;
                ...
                else
                {
                    ...
                    viewModel.NewSubItem = new UnitGroupSubModel()
                    {
                        GROUPID = viewModel.NewHostItem != null ? viewModel.NewHostItem.GROUPID : viewModel.SelectHostItem.GROUPID,
                        SYMBOL = viewModel.SelectItem.SYMBOL,
        ...
        private async void OnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    viewModel.SelectItem = viewModel.NewJoinItem != null ? viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.NewHostItem.GROUPID) :
                                                                           viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.SelectHostItem.GROUPID);
                    MasterContent.ScrollTo(viewModel.SelectItem, ScrollToPosition.Center, true); // Прокручиваем Scroll до активной записи.
                }
