...
namespace DECAPP.Views.Tools
{
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class ToolsCatalogPage : ContentPage
    {
        private ToolsList viewModel;

        public ToolsCatalogPage()
        {
            InitializeComponent();
        ...
        protected override async void OnAppearing()
        {
            ...
            try
            {
                ...
                BindingContext = viewModel = viewModel ?? new ToolsList();
        ...
        private async Task SaveHostItem()
        {
            try
            {
                if (viewModel.GetSelectHostItem() != null)
                {
                    ...
                    viewModel.SelectHostItem.TOOLSNAME = viewModel.SelectItem.TOOLSNAME;
                ...
                else
                {
                    ...
                    viewModel.NewHostItem = new ToolsModel()
                    {
                        TYPEID = viewModel.TypeList[picTYPENAME.SelectedIndex].TYPEID,
                        TOOLSNAME = viewModel.SelectItem.TOOLSNAME,
        ...
        private async Task SaveSubItem()
        {
            try
            {
                ...
                else
                {
                    ...
                    viewModel.NewSubItem = new ToolsSubModel()
                    {
                        TOOLSID = viewModel.NewHostItem != null ? viewModel.NewHostItem.TOOLSID : viewModel.SelectHostItem.TOOLSID,
        ...
        private async void OnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    viewModel.SelectItem = viewModel.NewJoinItem != null ? viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.NewHostItem.TOOLSID) :
                                                                           viewModel.Collection.FirstOrDefault(a => a.ID == viewModel.SelectHostItem.TOOLSID);
                    MasterContent.ScrollTo(viewModel.SelectItem, ScrollToPosition.Center, true); // Прокручиваем Scroll до активной записи.
                }
